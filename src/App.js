...
return (
  <div className="container mt-5" id="container-body">
 
  <div className="container mt-5">
        <h1 className="text-center text-primary mb-4" id='quiz'>Quiz</h1>

    {showFinalResult ? (
      <div className="final-results text-center">
        <h2 className='currentscore'>Current score: {score}</h2>
        <h1 className='finalresultson'>Final Results</h1>
        <h2 className='scoreOfTotal' id={score <=2 ? 'scoreOfTotalred' : 'scoreOfTotalgreen'}>
          {score} out of {questions.length} correct - ({((score / questions.length) * 100)}%)
        </h2>
        <button id='restart' onClick={() => restartGame()} className="btn btn-primary mt-3">
          Restart Game
        </button>
      </div>
    ) : (
      <div className="question-card">
      <h2 className="mb-3" id='questionNum'>
